...
export const formatDate = (value: string) => new Intl.DateTimeFormat('ar-SA-u-ca-gregory', {day:'numeric',month:'long',year:'numeric',timeZone:'Asia/Riyadh'}).format(new Date(value+'T12:00:00Z'));
...
export const navItems = layout.navigation.filter(n => layout.sections.some(s => s.id===n.target && s.enabled) && (n.target!=='news' || news.length>0));
